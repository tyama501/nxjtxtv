...
// 2024 T.Yamada
// Modified to output 8bit width bin file.
...
usage(void)
{
    ...
    "    -8     Output 8bit width bin file. Use only with -c\n"
	};
...
getopts(int *pac, char ***pav)
{
	...
	while (ac > 0 && av[0][0] == '-') {
		...
		case 's':			/* set encoding start*/
			...
			break;
		case '8':			/* output 8bit width bin file*/
			gen_8b = 1;
			break;
		default:
...
gen_c_source(PMWCFONT pf, char *path)
{
	...
	FILE *obfp;
	FILE *obof_fp;
	char obfile[256];    /* Binary file */
	char oboffile[256];  /* Offset file */
	int font_count = 0;  /* Font count */
	int i;
	...
	if (gen_8b) {
		strcpy(obfile, base_name(path));
		strcat(obfile, ".bin");

		obfp = fopen(obfile, "wb");
		if (!obfp) {
			fprintf(stderr, "Can't create %s\n", obfile);
			return 1;
		}
	...
	if (pf->offset) {
		...
		if (gen_8b) {
			putc(font_count, obof_fp);
			putc(font_count >> 8, obof_fp);
			putc(pf->firstchar, obof_fp);
			putc(pf->firstchar >> 8, obof_fp);
			putc(pf->size, obof_fp);
			putc(pf->size >> 8, obof_fp);
		}
		...
		for (i=0; i<pf->size; ++i) {
			...
			if (gen_8b) {
				putc(pf->offset[i], obof_fp);
				putc(pf->offset[i] >> 8, obof_fp);
			}
		}
		fprintf(ofp, "};\n\n");
	}
